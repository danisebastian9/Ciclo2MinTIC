...
public class Archivo {
    ...
    public void escribirArchivo(ArrayList<Estudiantes> lista, File arch1){
        try {
    ...
    public ArrayList<Estudiantes> leerArchivo(File arch1){
        ArrayList<Estudiantes> recuperar = new ArrayList<Estudiantes>();

        try {
            leer = new ObjectInputStream(new FileInputStream(archivo));
            recuperar = (ArrayList<Estudiantes>) leer.readObject();
            leer.close();
        } catch (Exception e) {
